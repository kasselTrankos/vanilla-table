...
const pagination = (elm, length, pagesShown, itemsPerPage, onPage)=>{
  ...
  const removeActive = (target) =>{
    ...
    let active = target.getElementsByClassName('active')[0];
    active.className = active.className.replace('active', '');
    active.getElementsByTagName('SPAN')[0].parentNode.removeChild(active.getElementsByTagName('SPAN')[0]);
  };
  ...
  for(let i =0; i<pages.length; i++)
  {
    ...
    a.onclick = function(event){
      ...
      removeActive(event.target.parentNode.parentNode);
