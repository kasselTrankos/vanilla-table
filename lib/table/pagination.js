...
const isLessThanEnd = (size, page=1, pagesShown=6, itemsPerPage=6)=>
  (page<(getTotalPages(size, itemsPerPage)-getMiddleFromPagination(pagesShown))
...
const getElementsInThisPage = (size, {itemsPerPage, page})=>
...
const getPages =(size, page=1, pagesShown=6, itemsPerPage=6)=>{
  ...
  let buttons =  _.range(Pages(size, itemsPerPage)).map(
    (item)=> {
    return {
      type: 'number',
